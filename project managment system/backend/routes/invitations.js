...
router.post('/project/:projectId/invite', auth, async (req, res) => {
  try {
    ...
    // Check if user is authorized (only admins can invite members)
...
router.get('/my-invitations', auth, async (req, res) => {
  try {
    ...
    const invitations = await Invitation.find({
      $or: [{ invitedUser: userId }, { invitedEmail: userEmail.toLowerCase() }],
...
router.post('/invitations/:invitationId/accept', auth, async (req, res) => {
  try {
    ...
    // Notify project members that someone joined
    try {
      const io = req.app.get('io');
      ...
      await notifyProjectMembers(project._id, userId, {
        ...
        message: `${user.name || 'A new member'} has joined the project: ${project.name}`,
        relatedUser: userId,
...
router.post('/invitations/:invitationId/cancel', auth, async (req, res) => {
  try {
    ...
    // Check authorization (only admins can cancel invitations)
    const project = await Project.findById(invitation.project);
...
router.post('/invitations/:invitationId/resend', auth, async (req, res) => {
  try {
    ...
    // Check authorization (only admins can resend invitations)
    const project = await Project.findById(invitation.project);
    const isAdmin = project.members.some(m => m.user.toString() === userId && m.role === 'admin');

    if (!isAdmin) {
      return res.status(403).json({ message: 'Only team admins can resend invitations' });
    }
...
router.post('/project/:projectId/members/:userId/remove', auth, async (req, res) => {
  try {
    ...
    // Check authorization (only admins can remove members)
