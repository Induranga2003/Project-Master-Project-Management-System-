...
router.get('/:id', auth, async (req, res) => {
  try {
    const task = await Task.findById(req.params.id).populate('assignee').populate('comments.user');
...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    const { title, description, status, priority, dueDate, startDate, endDate, estimatedHours, actualHours, assignee } = req.body;

    if (title) task.title = title;
    if (description) task.description = description;
    if (status) task.status = status;
    if (priority !== undefined) task.priority = priority; // Allow 'none' and other values
...
router.put('/admin/:id', auth, adminAuth, async (req, res) => {
  try {
    ...
    if (percentComplete !== undefined) task.percentComplete = percentComplete;
