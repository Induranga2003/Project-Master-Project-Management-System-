...
router.get('/project/:projectId', auth, async (req, res) => {
  try {
    ...
    const budgetUsage = {
      ...
      percentage:
        project.budget.estimated === 0 ? 0 : Math.round((project.budget.actual / project.budget.estimated) * 100)
    };
...
router.get('/dashboard/overview', auth, async (req, res) => {
  try {
    ...
    const stats = {
      ...
      overdueTasks: myTasks.filter(t => t.dueDate && new Date(t.dueDate) < new Date() && t.status !== 'completed')
        .length,
