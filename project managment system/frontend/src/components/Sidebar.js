...
export default function Sidebar({ onCreateProject }) {
  ...
  const menuItems = [
    ...
    { icon: '👤', label: 'Team Members', path: '/users' },
      ...
      <div className="p-6 flex items-center justify-between border-b border-gray-700">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-gradient-to-br from-lime-400 to-green-500 rounded-full flex items-center justify-center">
            <span className="text-slate-900 font-bold text-xs">PMM</span>
          </div>
          {!collapsed && <span className="text-xl font-bold">ProjectManagerMaster</span>}
        </div>
        <button
